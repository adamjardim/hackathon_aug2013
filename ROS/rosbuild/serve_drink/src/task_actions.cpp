#include <serve_drink/task_actions.h>
...
void taskActions::executeBackup(const serve_drink::BackupGoalConstPtr& goal)
{
	//backup 1 meter
	ros::Rate r(60);
...
void taskActions::executeRelease(const serve_drink::ReleaseGoalConstPtr& goal)
{
	...
	while (1){
	  ...
	  if( !lflag && count > 50 && fabs( leftGripperTransform.getOrigin().x() - preLeftGripperX ) > 0.0005 && fabs( leftGripperTransform.getOrigin().y() - preLeftGripperY ) > 0.0005 )
	  {
	  ...
	  //open right gripper if the coordinate frame of right gripper changes
          if( !rflag && count > 50 && fabs( rightGripperTransorm.getOrigin().x() - preRightGripperX ) > 0.0005 && fabs( rightGripperTransorm.getOrigin().y() - preRightGripperY ) > 0.0005 )
	  {
